...
if (typeof ($) === 'undefined') {
    alert("jQuery reference not found. ");
}
...
MK.WebAPI = {
    ...
    Retrieve: function (entityName, entityId, uri, callerId, successCallback, errorCallback, includeMetadataValues, eTag, unmodifiedCallback, async) {
        /// <summary>
        ...
        ////if length is greater than 2048 characters then execute using batch request (supported 2048)
        if ((getWebAPIPath() + entityName + "?" + uri).length >= 2000) {
            // execute using batch request 

            /// default parameter for batch operations
            var batchId = MK.WebAPI.batch.getRandomId();
            var changesetId = MK.WebAPI.batch.getRandomId();
            var batchRequestArray = [];
            batchRequestArray.push(MK.WebAPI.batch.getRetrieveRequest(entityName, entityId, uri, includeFormattedValues));

            if (batchRequestArray.length > 0) {
                //send data for batch operation
                MK.WebAPI.batch.Execute([], batchId, changesetId, function (result) {
                    successCallback(result[0])
                }, errorCallback, true, batchRequestArray);
            }
            return;
        }
                ...
                if (data != null) {
                    ...
                    if (!isFetchXML) {
                        if (data["@odata.nextLink"]) {
                            ...
                            MK.WebAPI.GetNextPage(data["@odata.nextLink"], callerId, includeMetadataValues, 5000, successCallback, errorCallback, resultData);
                        }
                        else {
                        ...
                        if (data["@Microsoft.Dynamics.CRM.fetchxmlpagingcookie"] != null) {
                            ...
                            MK.WebAPI.getNextPageFetchXML(entityName, originalFetch, callerId, includeMetadataValues, successCallback, errorCallback, pagingInfo.pageCokies, pagingInfo.pageNumber, response, async);
                        }
                        else {
                            var response = JSON.parse(JSON.stringify(data), dateReviver)
                            successCallback(response);
    ...
    getNextPageFetchXML: function (entityName, originalFetch, callerId, includeFormattedValues, successCallback, errorCallback, pageCookies, pageNumber, resultData, async, count) {
        /// <summary>
        /// 
        /// </summary>
    ...
    batch: {
        ...
        getAssociateRequest: function (contentId, changeSetId, parentContentId, childContentId, relationshipPropertyName, entityName, childEntityName) {
            /// <summary>
            ...
            if (MK.Validation.isGuid(childContentId)) {
                rel["@odata.id"] = getWebAPIPath() + childEntityName + "(" + childContentId + ")";
            } else {
        ...
        getRetrieveRequest: function (entityName, entityId, uri, includeFormattedValues) {
            /// <summary>
            /// 
            /// </summary>
            ...
            payload.push("Content-Transfer-Encoding:binary");
            payload.push("");
            payload.push("GET " + encodeURI(getWebAPIPath() + uri) + " HTTP/1.1");
            if (includeFormattedValues) {
                payload.push("Prefer: odata.include-annotations=\"OData.Community.Display.V1.FormattedValue\"");
            }
            payload.push("Content-Type:application/json;type=entry");
            payload.push('');
            payload.push('{}');
        ...
        Execute: function (payload, batchId, changeSetId, successCallback, errorCallback, async, retrievePayload) {
            /// <summary>
            /// 
            /// </summary>
            ...
            var batchHeader = "--batch_" + batchId + "\n";
            var batchFooter = "\n--batch_" + batchId;
            ...
            var changesetFooter = []
            changesetFooter.push("--changeset_" + changeSetId + "--");
            changesetFooter.push("");

            if (payload && payload.length > 0) {
                payload = changesetHeader.join("\r\n") + payload.join("\r\n") + changesetFooter.join("\r\n");
            }
            else {
                payload = "";
            }
            ...
            //for retrieve payload
            var retrieveHeader = ["--batch_" + batchId + "\n"]
            var retrieveFooter = ["\n--batch_" + batchId];
            if (retrievePayload && retrievePayload.length > 0) {
                retrievePayload = retrieveHeader.join("\r\n") + retrievePayload.join("\r\n") + retrieveFooter.join("\r\n");
            }
            else {
                retrievePayload = "";
            }

            if (payload && retrievePayload) {
                payload = batchHeader + payload + retrievePayload;
            }
            else if (payload) {
                payload = batchHeader + payload + batchFooter;
            }
            else {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, xhr) {
                    if (textStatus === "timeout") {
                        ...
                        if (successCallback) {
                            if (MK.WebAPI.batch.isResponseError(data)) {
                                var message = MK.WebAPI.batch.parseResponseError(data);
                                var errorMsg = new Error(message);
                                errorCallback(errorMsg);
                                return;
                            }
                            successCallback(MK.WebAPI.batch.parseBatch(data));
                        }
                    }
                },
                error: function (xhr, textStatus, errorThrown) {
        ...
        getDisassociateRequest: function (contentId, changeSetId, entityName, entity1Id, entity2Id, relationshipName) {
            /// <summary>Delete an entity</summary>
            /// <param name="uri" type="String">The Uri for the entity you want to delete</param>        
            /// <param name="entity1Id" type="Guid">
            /// If it is N:1 then it is child entity id otherwise parent entity id</param>
            /// <param name="entity2Id" type="Guid">
            /// If it is N:1 then pass null otherwise child entity id</param>
            /// <param name="relationshipName" type="Guid">
            /// If it is N:1 then pass referencingEntityNavigationPropertyName (use metadata browser to get relationshipName) otherwise ReferencedEntityNavigationPropertyName.</param>
            /// <param name="successCallback" type="Function">The function to call when the entity is deleted.</param>
            /// <param name="errorCallback" type="Function">The function to call when there is an error. The error will be passed to this function.</param>
            /// <param name="callerId" type="Guid" optional="true" optional="true">The systemuserid value of the user to impersonate</param>

            if (!MK.Validation.isGuid(entity1Id)) {
                throw new Error("MK.WebAPI.batch.getDisassociateRequest entity1Id parameter must be a string and GUID.");
            }

            if (!MK.Validation.isString(entityName)) {
                throw new Error("MK.WebAPI.batch.getDisassociateRequest entitySetName parameter must be a string.");
            }

            if (entity2Id && !MK.Validation.isGuid(entity2Id)) {
                throw new Error("MK.WebAPI.batch.getDisassociateRequest entity2Id parameter must be a Guid or null.");

            }
            if (!MK.Validation.isString(relationshipName)) {
                throw new Error("MK.WebAPI.batch.getDisassociateRequest relationshipName parameter must be a string.");
            }

            var uri = null;
            if (entity2Id) {
                uri = uri = getWebAPIPath() + entityName + '(' + entity1Id + ')' + "/" + relationshipName + "(" + entity2Id + ")/$ref";
            }
            else {
                uri = getWebAPIPath() + entityName + '(' + entity1Id + ')' + "/" + relationshipName + "/$ref";
            }
            //var uri = getWebAPIPath() + entityName + '(' + entity1Id + ')' + "/" + referencingEntityNavigationPropertyName + "/$ref";
        ...
        parseBatch: function (responseCollection) {
            var boundary = '--batch_';
            var items = [];
        ...
        isResponseError: function (response) {
            /// check batch response contains error or not
            "use strict";
        ...
        parseResponseError: function (response) {
            /// get error message from batch response        
            "use strict";
            if (response) {
                return response.substring(response.indexOf("message") + 10, response.indexOf("innererror") - 3);
            }
            return "";
        }
    },
    ...
    invokeFunction: function (entityName, entityId, functionName, parameter, successCallback, errorCallback, callerId) {
        if (functionName.indexOf("Microsoft.Dynamics.CRM") < 0) {
            functionName = "Microsoft.Dynamics.CRM." + functionName
        }
        var uri = getWebAPIPath() + entityName + "(" + entityId + ")/" + functionName;
        if (parameter) {
            uri += parameter
        }
        var req = new XMLHttpRequest();
    ...
    GetVersion: function () {
        "use strict";
        MK.WebAPI.Retrieve("organizations", null, "$select=initialversion,name", null, function (result) {
            result = result.value[0];
            var version = result["initialversion"].split(".");
            if (version.length > 2) {
                MK.Organization.version = version[0] + "." + version[1];

                if (Number(version[0]) >= 9) {
                    MK.Organization.linkEntityPrefix = ".";
                }
            }
            MK.Organization.name = result["name"];
        }, function (errorMsg) {
            alert(errorMsg);
        }, null, null, null, true);
    }
...
function getWebAPIPath() {
    return getClientUrl() + "/api/data/v" + MK.Organization.version + "/";
}
...
Date.prototype.toUTC = function () {
    "use strict";
    return new Date(this.getUTCFullYear(), this.getUTCMonth(), this.getUTCDate(), this.getUTCHours(), this.getUTCMinutes(), this.getUTCMilliseconds());
};
...
function toStringFromXML(xml) {
    return new XMLSerializer().serializeToString(xml);
};

function isXML(xmlStr) {
    try {
        toStringFromXML(xmlStr);
    } catch (e) {
        return false;
    }
    return true;
...
//get CRM/D365 version 
MK.WebAPI.GetVersion();
